...
to.manuscript = "~/Documents/Research/1.Projects/Eawag/Projects/8.Blue-Green/5.Redaction/1.Turnover/5.ProcB/"
...
##################
#Figure panel with first and last dates for each replicate 
##################
...
pdf(paste0(to.manuscript,"PROT_NET_COMP",".pdf"),width=5,height=10)
...
for(i in 1:nlevels(dates)) {
  for(j in 1:nlevels(Prot.b$Replicate)) {
      ...
      rel.ab = Prot.b[which(Prot.b$date == levels(dates)[i] & Prot.b$Treatment=="Isolated" & Prot.b$Replicate==levels(Prot.b$Replicate)[j]),species[-8]]
      ...
      Psize = Prot.b$Size[which(Prot.b$date == levels(dates)[i] & Prot.b$Treatment=="Isolated" & Prot.b$Replicate==levels(Prot.b$Replicate)[j])]
      ...
      plot.igraph(eval(parse(text= paste("Rep",levels(Prot.b$Replicate)[j],".","g",sep=""))),
                  ...
                  vertex.label.font=2,
                  main=paste0("Day"," ",levels(day)[i]," ; ","Rep"," ",levels(Prot.b$Replicate)[j]))
    }
  }
...
##################
#Generate figures for each date of each replicate for the two tretments (connected and isolated) - each figure is in its nown pdf file
##################
